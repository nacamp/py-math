...
class FieldExtBase(tuple):
    ...
    #         return NotImplemented
    #
    #     # Fq6.Q=11, Fq2.Q=7 이라면 이곳에서 Fq6 buf에 넣을때 Q=11로 처리한다.
    #     buf = [cls.basefield.zero(self.Q) for _ in self]
    #     #print(self.Q, len(self), len(other), self, other)
    #     for i, x in enumerate(self):
    #         if cls.extension == other.extension:
    #             for j, y in enumerate(other):
    #                 if not isinstance(y, Fq):
    #                     print(f'i-j:{i}-{j}-{x}-{y}')
    #                 if x and y:
    ...
    #     ret.Q = self.Q
    #     ret.root = self.root
    #     print(' <',self.Q, self, other, 'result = ', ret)
    #     return ret
    def __mul__(self, other):
        cls = type(self)
        if DEBUG:
        ...
        for i, x in enumerate(self):
            if cls.extension == other.extension:
                for j, y in enumerate(other):
                    if DEBUG and not (isinstance(y, Fq) or isinstance(y, Fq2)):
                        print(f'i-j:{i}-{j}-{x}-{y}')
                    if x and y:
                        if i + j >= self.embedding:
                            if DEBUG and  not (isinstance(y, Fq) or isinstance(y, Fq2)):
